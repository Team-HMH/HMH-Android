...
class CustomResultCall<T>(
    ...
    override fun clone(): Call<Result<T>> {
        return CustomResultCall(call.clone(), retrofit)
    }
    ...
    override fun enqueue(callback: Callback<Result<T>>) {
        ...
        call.enqueue(object : Callback<T> {
                ...
                if (response.isSuccessful) {
                    ...
                    if(response.body() == null) {
                        callback.onResponse(
                            ...
                            Response.success(Result.failure(CustomCallException("응답 body가 비었습니다.", HttpException(response))))
                        )
                    }
                    // HTTP 요청 성공
                    else {
                    ...
                    // 요청에 실패한 경우
                        ...
                        // CustomCallException 객체 생성
                        callback.onResponse(this@CustomResultCall,
                            Response.success(Result.failure(CustomCallException(message, HttpException(response))))
                        )

                        Timber.tag("ResultCall - onResponse").e("${CustomCallException(message, HttpException(response))}")
                    }
                }
            }
            ...
            override fun onFailure(call: Call<T>, t: Throwable) {
                ...
                callback.onResponse(
                    this@CustomResultCall,
                    Response.success(Result.failure(CustomCallException(message, t)))
                )

                Timber.tag("ResultCall - onFailure").e("${CustomCallException(message, t)}")
            }
        })
    }
}